...
## 🎮 友達 (Switch/PS5) の参加方法 (推奨: MCXboxBroadcast)
**Switch勢などの参加には、これをお勧めします。**
Xboxアカウントでフレンドになるとフレンド欄にサーバーが出現するというものです。
本来はGeyserMC向けのものですが、このサーバー(BDS)でも単体で動かせます。

### 導入手順
1. **ツールをダウンロード**:
   - [MCXboxBroadcast](https://github.com/MCXboxBroadcast/Broadcaster) の Releases から `MCXboxBroadcastStandalone.jar` をダウンロードします。
2. **サブ垢の用意**:
   - サーバー宣伝用のXboxアカウント（サブ垢）を1つ用意します。
3. **起動と認証**:
   - ダウンロードした `.jar` ファイルを実行します（Javaが必要です）。
   - 初回起動時に表示される認証コードを使って、用意したサブ垢でログインします。
4. **フレンド登録**:
   - 参加したい友達は、その「サブ垢」にフレンド申請を送ります。
   - 承認されると、Switchなどのマイクラの「フレンド」タブにこのサーバーが表示されるようになります！
