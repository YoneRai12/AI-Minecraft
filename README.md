...
### 6. 🎥 観戦・可視化ツール (New!)
- **自動ドローンカメラ (Camera Director)**:
    - 死亡したプレイヤー、または `cameraman` タグを付けたプレイヤーは、自動的に「生存者を追尾するドローンカメラ」モードになります。
    - **操作方法 (Discord)**: 死者へ届くDMの「Next」「Stop」ボタンで操作可能。
    - **操作方法 (Chat)**: マイクラのチャットで `!next` (次へ)、`!stop` (終了) と打つことでも操作できます（スマホ版に最適）。
- **3Dデバッグビューア (Web)**:
